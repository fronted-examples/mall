<template>
    <section class="chat-room flex-row">
      <div class="side">
      ...
      <div class="room">
        ...
        <room-tool 
            ref="room-tool" 
            :callable="contactor.userId !== (userInfo && userInfo.userId)"
            :on-upload-file="fileChange"
            @change="onSelectMessageType" 
            @video-call="onVideoCall" 
            @hang="onVideoHang" 
            @candidate="onVideoCandidate" 
            @offser="onVideoOffer" />
  ...
  import { Tabbar, AddressBook, RoomHeader, RoomContent, RoomTool, RoomFooter, MessageItem } from '@/components/Chat'
